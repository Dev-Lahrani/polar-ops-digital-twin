...
export default function PredictiveMaintenancePage() {
  ...
  const getTrendIcon = (trend: string) => {
    switch (trend) {
      ...
      case "improving": return <TrendingUp className="w-3.5 h-3.5 text-green-400" />;
    ...
    <div className="min-h-screen bg-[var(--background)] p-6">
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-mono text-white tracking-wider">PREDICTIVE MAINTENANCE</h1>
            <p className="text-sm text-slate-400 mt-1">Weibull reliability analysis &amp; MTBF forecasting</p>
          </div>
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="bg-[var(--surface)] border border-[#1e293b] rounded-lg p-4">
            <div className="text-xs font-mono text-slate-500 mb-1">FLEET RELIABILITY</div>
            <div className="text-3xl font-mono text-white">{result.fleetReliabilityScore}%</div>
            <div className="mt-2 h-2 bg-[#0d1117] rounded-full overflow-hidden">
              <div
                className={`h-full rounded-full transition-all ${
                  result.fleetReliabilityScore >= 70 ? "bg-green-500" :
          ...
          <div className="bg-[var(--surface)] border border-[#1e293b] rounded-lg p-4">
            <div className="text-xs font-mono text-slate-500 mb-1">COMPONENTS MONITORED</div>
            <div className="text-3xl font-mono text-white">{result.components.length}</div>
          ...
          <div className="bg-[var(--surface)] border border-[#1e293b] rounded-lg p-4">
            <div className="text-xs font-mono text-slate-500 mb-1">UPCOMING MAINTENANCE</div>
        ...
        <div className="bg-[var(--surface)] border border-[#1e293b] rounded-lg p-4">
          <h2 className="font-mono text-sm text-slate-300 mb-4">COMPONENT RELIABILITY MATRIX</h2>
          <div className="overflow-x-auto">
            <table className="w-full text-xs font-mono">
              <thead>
                <tr className="border-b border-[#1e293b]">
                  <th className="px-3 py-2 text-left text-slate-500">Component</th>
                  <th className="px-3 py-2 text-left text-slate-500">Category</th>
                  <th className="px-3 py-2 text-center text-slate-500">Reliability</th>
                  <th className="px-3 py-2 text-center text-slate-500">MTBF (hrs)</th>
                  <th className="px-3 py-2 text-center text-slate-500">Hours Run</th>
                  <th className="px-3 py-2 text-center text-slate-500">Trend</th>
                  <th className="px-3 py-2 text-center text-slate-500">Risk</th>
                </tr>
              </thead>
              <tbody>
                {result.components.sort((a, b) => a.reliabilityScore - b.reliabilityScore).map(comp => (
                  <tr key={comp.id} className="border-b border-[#1e293b]/50 hover:bg-[#0d1117]/50">
                    <td className="px-3 py-2 text-white">{comp.name}</td>
                    <td className="px-3 py-2 text-slate-400">{comp.category}</td>
                    <td className="px-3 py-2 text-center">
                      <span className={comp.reliabilityScore < 50 ? "text-red-400" : comp.reliabilityScore < 70 ? "text-yellow-400" : "text-green-400"}>
                        {comp.reliabilityScore}%
                      </span>
                    </td>
                    <td className="px-3 py-2 text-center text-slate-300">{comp.mtbf.toLocaleString()}</td>
                    <td className="px-3 py-2 text-center text-slate-300">{comp.operatingHours.toLocaleString()}</td>
        ...
        <div className="bg-[var(--surface)] border border-[#1e293b] rounded-lg p-4">
          <h2 className="font-mono text-sm text-slate-300 mb-4">MAINTENANCE TIMELINE</h2>
            ...
            {result.maintenanceTimeline.slice(0, 8).map((entry, i) => (
              <div key={i} className={`flex items-center gap-3 px-3 py-2.5 rounded border ${getUrgencyColor(entry.urgency)}`}>
                ...
                <div className="flex-1">
                  <div className="text-sm text-white">{entry.componentName}</div>
                ...
                <div className="text-right">
                  <div className={`text-sm font-mono font-bold ${
                    ...
                    entry.daysFromNow < 90 ? "text-yellow-400" : "text-slate-400"
                  }`}>
                    {entry.daysFromNow < 0 ? `${Math.abs(entry.daysFromNow)}d overdue` : `in ${entry.daysFromNow}d`}
                  </div>
                  <div className="text-[10px] text-slate-500 uppercase">{entry.urgency}</div>
                </div>
              </div>
            ))}
          </div>
